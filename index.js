 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`ARDcore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`ARDcore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'ARDcore-' + name : 'ARDcore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/ARDcore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = browserifyPath + ' --require ./index.js:' + fullname + ' --external ARDcore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = browserifyPath + ' --require ./index.js:ARDcore-lib -o ARDcore-lib.js';
    }
